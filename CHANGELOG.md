...
**상태: 안정 선언 전.** `AI-START.md`의 절차와 출력 양식이 판마다 바뀌고 있습니다. 표시(`naite ID · 판`)와 항목 ID는 판이 바뀌어도 그대로 유효하지만, 진단 절차 자체를 팀 규칙으로 굳히는 것은 안정 선언 뒤를 권합니다.
...
- `chat.md` 신설 — 채팅 AI를 쓰는 사람이 직접 골라 붙여 넣는 문장 모음. 파일·설정·스크립트 없이 성립하는 것만: starter S3, spec 12, numbers 6과 체크리스트 안내, setup 3, 도구 없이 쓰는 kit 9종. 표시 예시에 starter 항목(S3)과 여러 줄 문장의 표시 위치, 판 번호만 보면 된다는 안내
- README — 채팅 절을 `chat.md` 안내로 줄임, 거둠 안내의 "판단형·사건형"을 풀어 씀, 용어 절(판 · gate · 상시 지침 · 사건형/판단형), 개인 작업 환경 범위 선언, `cases.md` 안내
- `starter.md` — 머리에 채팅 사용자 안내
- 4개 트랙 `gates.md` — 머리에 전제 환경 한 줄
- `AI-START.md` — 표시의 HTML 주석 형식(`<!-- naite SPEC-04 · v0.1.0 -->`)을 괄호 형식과 같은 표시로 인정, 두 형식과 starter 표시를 함께 찾는 검색 패턴
- `retire.md` — 주석 형식 예시
- `adapters/claude-code.md` — 주석 형식 메모(도구가 주석을 모델에 전달하는지는 확인하지 않음)
- CHANGELOG — 상태 한 줄(안정 선언 전), automation 번호 공백 사유
...
- README — 채팅 AI 사용법이 spec 트랙만 가리키던 것을 네 트랙(하는 일 → 볼 `gates.md` 표)으로 고침(ID 변경 없음)
...
**추가 ID:** AUTO-01 · AUTO-02 · AUTO-03 · AUTO-04 · AUTO-05 · AUTO-06 · AUTO-09 · AUTO-14 · AUTO-15 · AUTO-16 · AUTO-17 · AUTO-18 · AUTO-19 · AUTO-20 · AUTO-21 · AUTO-22 · AUTO-23 · NUM-01 ~ NUM-28 (AUTO-07 · 08 · 10 · 11 · 12 · 13은 이 판에 넣지 않은 번호 — 운영체제에 딸린 사실이라 제외)
